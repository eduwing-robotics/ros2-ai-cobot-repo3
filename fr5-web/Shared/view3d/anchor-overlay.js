 ...
 *
 * `yawOffsetDeg` — 종이 방위에 **더하는** 관례 각. 컨베이어2(32)는 작업대2 의 실물 지그를
 * 가리면 안 되고 작업대3 쪽 세로로 서야 한다 — 종이는 그대로 두고 90° 만 꺾는다.
 */
export const ANCHOR_PROPS = {
  // `anchor: 'end'` — 태그가 **끝**이고 몸은 진행방향으로 뻗는다 (긴 물건 전용).
  // 없으면 태그가 **중심**이다 — 지그처럼 정사각에 가까운 것은 중심이 자연스럽다.
  32: { type: 'conveyor', yawOffsetDeg: 90, anchor: 'end',                  // 컨베이어2 · 배출 — 세로
  ...
  //
  // ⚠ **다시 켜려면**: 실물이 없어진 자리에만 켠다. 지금 이 표에 남은 것은 컨베이어 둘로,
  //    그건 아직 실물이 없다.
};
