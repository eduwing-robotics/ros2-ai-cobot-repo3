...
// ─────────────────────────────────────────────────────────────────────────────
// 조립 라인 — **실기 담당자의 맵 1 을 정본으로 삼는다** (2026-08-04 · `Downloads/1.json`).
//
//   남(y=0)                                                        북(y=12000)
//   ┌ 벽 ────────────────────────────────────────────── 개구부 ─┐
...
//   · 컨베이어·거치대 **자리** — 실기 담당자 구상(33 일자 → jig2 조립 → 32 세로줄 → 작업대3)을 판 위에
...
//     「실기 담당자 구상을 판 위에 그린 값」이다. 태그 32·33 을 놓고 `anchor-pose.py` 로 풀면
...
const realmap = () => ({
  ...
  props: [
    ...
    //    **일자 2개가 90° 로 만난다** — ㄱ자 토막·곡선 부품은 비채택 (D135 · 실기 담당자 2026-08-19
    ...
    //    먹어 공장 설비처럼 보였고(실기 담당자 08-19), 150×100 도 실물 지그를 가려 **20% 더 줄였다**
    //    (같은 날 2차 지시 · cam.js `ANCHOR_PROPS` 와 같은 값 — 트윈과 AR 이 같은 크기를 그린다)
    //    `belt` — 평벨트. 77mm 탄은 롤러 틈(피치 100)에 빠진다 (실기 담당자 08-19 3차)
    ...
    // ── 가상 지그는 뺐다 (실기 담당자 08-19 2차) — **거치대는 실물이 온다.** 실물 자리는 태그
    //    15·18·21 을 붙여 비전이 말하게 한다 (D130 과 같은 규칙 — 그림으로 겹쳐 그리지 않는다)
  ],
});

// ─────────────────────────────────────────────────────────────────────────────
// 방산 조립 라인 — **연출 무대다** (2026-08-20 · 실기 담당자 지시)
//
...
// ⛔ **터틀봇 통로와 스테이션은 같은 선 위에 있어야 한다** (2026-08-20 · 실기 담당자가 화면에서 잡음).
...
//
// ⛔ **팔은 통로와 컨베이어 사이에 선다** (2026-08-20 · 실기 담당자: 「팔이 AMR 에 닿은 다음
...
const defenseLine = () => ({
  ...
  windows: [],
  // ⛔ **팔은 둘이다. 셋이면 동작이 어색해진다** (2026-08-20 · 실기 담당자가 화면에서 잡음).
  ...
  // ⭐ **자리가 두 줄로 갈린다** (2026-08-20 · 실기 담당자: 「안 보이는 터틀봇을 사용하자」).
  ...
  // **통로 한 줄 직진뿐이다** (실기 담당자 2026-08-20). 컨베이어 줄 자리로 가야 하는 구간은
  // 경로 밖이라 화면이 터틀봇을 **안 그린다** — 위 §자리 두 줄 참조
  amrs: [AMR('amr1', [LX, 1400], [[LX, 1400], [LX, 4200], [LX, 7900]], { laneOnly: true, robotId: 'tb3_2' })],
  // 소품 배치는 **`cell`(맵 1 정본 · 실기 담당자) 의 문법을 그대로 따른다** (실기 담당자 지시 2026-08-20
  ...
  props: [
    ...
    // ⛔ 크레인은 안 쓴다 (실기 담당자 2026-08-20) — 실셀에 없는 설비이고, 화면 위를 덮어
