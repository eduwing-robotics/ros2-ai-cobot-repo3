// 싣기 9칸 — **정본은 여기 하나다** (2026-09-06 · `SIM-TAB-CONVERGE-LOOP` phase 1).
//
...
/**
 * 싣기 9칸. `stop` 은 정차 자리(`AMR_DROP` 또는 후보). 각 칸이 「왜 거기인가」를 든다.
...
export function makeLoadSteps(stop = AMR_DROP, { tableTopZMm = AMR_HOME.topZMm, graspXyMm = null, graspRzDeg = null, basket = null } = {}) {
  ...
  const at = (x, y, z, rz) => [x, y, z, g[3], g[4], rz];
  ...
  const bodyOff = carrierBodyOffset(observed ? rzPlace : g[5]);
  const bx = bx0 - bodyOff.dxMm;
  const by = by0 - bodyOff.dyMm;
  const floor = basketFloorZ();
  const graspLift = CARRIER_GRASP_TRUTH.tcpAboveTableMm;      // 손끝이 거치대 바닥에서 얼마 위인가 — 실측 14.9
  ...
  return [
    ...
    { id: 'grasp', label: '② 무는 자세', grip: 100, pose: at(g[0], g[1], g[2], rzGrasp), why: '사람이 손으로 맞춘 자세 (실측 정본 · 높이는 그려진 상판 기준)' },
