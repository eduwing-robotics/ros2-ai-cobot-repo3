...
| `src/features/sim/automatic/` | 시뮬 탭의 **S0~S4 제한 자동 실행** 경계 — 단계 상한·중단·HOLD와 S4 고스트 `PREVIEW` 확인 화면. S3은 미래 정차·놓기 전체 계획, S3R은 현재→높은 접근·100→70% 사전 성형, S4는 전 경로를 먼저 검증하고 고스트 확인 뒤 `approach→pregrip→grasp→close→lift`만 기존 실행 콜백으로 잇는다. 터틀봇 주행은 소유하지 않는다 |
