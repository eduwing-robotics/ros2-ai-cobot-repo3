...
# 그리퍼. 시작할 때 항상 35 로 벌리고, 하강을 마치면 25 로 잡는다 (탄두는 17).
GRIPPER_ID    = 1
GRIPPER_START = 35
GRIPPER_GRIP  = 25
...
KIND_CFG = {
    ...
    "탄두": {
        # 2026-09-11 실측. 정렬 기준 (286.748, -1160.084, -86.586) 에서 Jog 으로
        # 파지 지점까지 옮겨 잰 값. 자세 차이 0.000도로 순수 병진이다.
        # 탄피(+34.94, -86.46, -194.23) 보다 Z 가 6.76mm 더 내려간다 —
        # 탄두 15mm / 탄피 21mm 의 길이 차이와 맞는다.
        "grip":     [38.23, -87.87, -200.99],
        # 2026-09-11 Jog 실측. 탄피를 놓은 자리 위 — 탄피 경유점과 X/Y 가 1~2mm 차이다.
        # 참고 관절 = [58.799, -56.015, 80.881, -114.611, -90.010, -30.502]
        "approach": [127.630, -1146.598, -239.396, 179.770, 0.121, 179.300],
        # 경유점에서 Z 만 6.00mm 내려간 자리 (dx 0.000 / dy -0.002 로 순수 수직).
        # 참고 관절 = [58.799, -55.667, 81.262, -115.340, -90.010, -30.502]
        "pose":     [127.630, -1146.600, -245.397, 179.770, 0.121, 179.300],
        # 놓을 위치에서 1.16mm 더 눌러 앉힌다 (실측 Z=-246.553,
        # 참고 관절 = [58.799, -55.599, 81.334, -115.479, -90.010, -30.502])
        "press":    [-1.16],
        # 2026-09-11. 박기 마지막 좌표에서 -Y 로 2mm 더 간다.
        "after":    [(0.0, -2.0, 0.0)],
        "close":    17,              # 탄두가 탄피보다 얇아 더 조인다 (탄피는 22)
        "release":  False,           # 파지를 풀지 않는다 — 쥔 채로 멈춘다
        "back":     False,           # 놓은 자리에서 멈춘다 (확인용)
    },
}
...
ap.add_argument("--detector", default="sam3", choices=["sam3", "depth", "both"],
...
if a.grip_offset:
    ...
    GRIP_JSON = json.load(open(GRIP_FILE))["dxyz"]
...
def detect(bgr, depth):
    ...
    for mk in masks:
        ...
        if aspect < a.pose_aspect:
            pose = "꽂힘"
            # 거리로 가르는 것이 1순위다. 같은 평면에 꽂혀 있으면 긴 쪽(탄피)이
            # 카메라에 가깝다. 09-11 실측으로 17mm 이상 벌어져 길이(1.5mm 차)보다 안전하다.
            if a.z_split_mm > 0 and not np.isnan(Zc) and Zc > 0:
...
def settle(tgt_xyz, tol, timeout, still=0.05, need=3, tick=0.05):
    ...
    t0 = time.time(); prev = None; quiet = 0
    while time.time() - t0 < timeout:
        spin()
        cur = cur_pose()
        if prev is not None and float(np.linalg.norm(cur - prev)) < still:
            quiet += 1
        else:
            quiet = 0
        prev = cur
