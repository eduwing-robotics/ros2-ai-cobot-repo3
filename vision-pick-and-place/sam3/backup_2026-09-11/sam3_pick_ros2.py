...
# 그리퍼. 시작할 때 항상 35 로 벌리고, 하강을 마치면 22 로 잡는다.
GRIPPER_ID    = 1
GRIPPER_START = 35
GRIPPER_GRIP  = 22
...
# 놓을 위치 — 절대 카테시안 (x, y, z, rx, ry, rz) mm/도. 2026-09-11 Jog 실측.
# 참고 관절 = [58.691, -54.787, 84.651, -122.595, -89.519, -29.800]
PLACE_POSE = [126.144, -1150.382, -269.579, -177.395, -0.940, 178.480]

# 놓을 위치로 가기 전 경유점. 2026-09-11 Jog 실측 (놓을 위치보다 92mm 위).
# 참고 관절 = [58.691, -59.928, 78.395, -111.198, -89.519, -29.800]
#
# 경유점 없이 파지 위치에서 놓을 위치로 곧장 카테시안 이동하면 경로가 나빠진다 —
# 09-10 에 목표에서 626mm 어긋나 비상정지했고, 급정지 중 6축이 슬레이브에서
# 탈락해 컨트롤 박스 전원 재투입이 필요했다. 경유점을 넣은 09-11 실행은
# 목표에 2mm 안으로 도달했다.
PLACE_APPROACH = [126.144, -1150.390, -177.395, -177.396, -0.940, 178.480]
...
ap.add_argument("--simultaneous", action="store_true",
                help="X·Y를 동시에 맞춘다. 기본은 X 먼저 맞추고 그다음 Y")
...
GRIP = None
if a.grip_offset:
    GRIP = [float(v) for v in a.grip_offset.split(",")]
    print(f"보정값: 인자에서 읽음 ({GRIP[0]:+.1f}, {GRIP[1]:+.1f}, {GRIP[2]:+.1f}) mm")
elif os.path.exists(GRIP_FILE):
    GRIP = json.load(open(GRIP_FILE))["dxyz"]
    print(f"보정값: {os.path.basename(GRIP_FILE)} 에서 읽음 "
          f"({GRIP[0]:+.1f}, {GRIP[1]:+.1f}, {GRIP[2]:+.1f}) mm")
...
if not (a.save_grip_offset or a.no_servo):
...
def show(bgr, dets, tgt, note=""):
    ...
    cv2.putText(vis, note, (8,20), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0,255,255), 2)
...
def move_rel(dx, dy, dz=0.0, wait=True, tol=2.0, timeout=None, speed=None):
    ...
    while time.time()-t < timeout:
        ...
        s_ = state.get("s")
        if s_ is not None:
            d = np.linalg.norm([s_.cart_x_cur_pos-tgt_xyz[0], s_.cart_y_cur_pos-tgt_xyz[1],
                                s_.cart_z_cur_pos-tgt_xyz[2]])
...
def lift():
    ...
    if a.lift_mm <= 0:
        print("\n[들어올리기] --lift-mm 0 이라 올리지 않습니다")
        return True
    p0 = cur_pose()
    print(f"\n[들어올리기] 상대 이동 (0, 0, {a.lift_mm:+.1f}) mm")
    if not move_rel(0.0, 0.0, a.lift_mm, speed=a.speed_move):
...
def move_abs(pose, label, tol=3.0, timeout=None, speed=None):
    ...
    while time.time()-t < timeout:
        spin()
        cur = cur_pose()
        if np.linalg.norm(cur - np.array([x, y, z])) < tol:
...
def move_home():
    return move_joints(HOME_JOINTS, "시작 위치로 이동")


def move_place():
    """놓을 위치 위 50mm 로 간 뒤 수직으로 내려가 놓는다."""
    if not move_abs(PLACE_APPROACH, "경유점으로 이동"):
        print("  ✗ 접근 지점 이동 실패 — 물체를 잡은 채로 멈췄습니다")
        return False
    if not move_abs(PLACE_POSE, "놓을 위치로 하강"):
...
try:
    ...
    if a.save_grip_offset:
        ...
        json.dump({"dxyz": _d, "from": _ref,
                   "measured_tcp": [round(float(v), 2) for v in _now]
                                   + [round(float(v), 2) for v in _rpy],
                   "time": time.strftime("%H:%M:%S")},
                  open(GRIP_FILE, "w"), indent=1, ensure_ascii=False)
        print(f"\n정렬 기준 ({_ref['kind']}) : "
        ...
        # 자세가 같아야 순수 병진이다. 회전이 섞이면 상수로 쓸 수 없다.
        _dr = [abs(_rpy[i] - _ref["tcp"][3+i]) for i in range(3)] if len(_ref["tcp"]) >= 6 else None
        if _dr and max(_dr) > 0.5:
            print(f"  ⚠️ 자세가 {max(_dr):.2f}도 달라졌습니다 — 순수 병진이 아니라 회전이 섞였습니다.")
            print("     Jog 을 병진만으로 다시 하세요. 관절 차이는 보정값으로 쓸 수 없습니다.")
        sys.exit(0)

    if a.no_servo:
        if GRIP is None:
            sys.exit("보정값이 없습니다. --grip-offset \"dx,dy,dz\" 를 주거나 "
                     "grip_offset.json 을 두세요.")
        gx, gy, gz = GRIP
        aligned = cur_pose()
        ...
        print(f"\n[그리퍼] {GRIPPER_GRIP} 으로 잡습니다")
        gripper(GRIPPER_GRIP, force=True, expect_hold=True)
        if not lift():
            sys.exit(1)
        if not a.no_place and not move_place():
            sys.exit(1)
    ...
    for d in sorted(dets, key=lambda d:(0 if d['kind']=='탄피' else 1)):
        ...
        _mark = ""
        # 길이가 경계 ±1.5mm 안이면 길이를 못 믿고 중심 밝기로 판정한다 → 흔들리는 구간
        if not np.isnan(_L) and abs(_L - a.len_split_mm) < 1.5: _mark = "  ← 경계, 밝기로 판정"
        print(f"   - {d['kind']}({d['pose']}) pix=({d['u']:.0f},{d['v']:.0f}) "
              f"Z={d['Z']:.0f}mm L={_L:.1f}mm{_mark}")
    ...
    def servo(axis, label):
        ...
        while True:
            ...
            if tgt is None:
                if not wait_for_target("정렬 중 대상이 화면에서 사라졌습니다"):
                    return False
    ...
    if a.descend:
        ...
        print(f"[하강 완료] 목표와의 차이 = "
              ...
              f"{_p[2]-(aligned[2]+gz):+.2f}) mm")
        print(f"\n[그리퍼] {GRIPPER_GRIP} 으로 잡습니다")
        gripper(GRIPPER_GRIP, expect_hold=True)
