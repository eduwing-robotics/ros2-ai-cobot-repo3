...
def move_wobj_target(robot, target_pose, label):
...
def main():
    ...
    try:
        ...
        print()
        print("========================================")
        print("SAM3 PICKUP -> APRILTAG PLACEMENT")
        print("========================================")
        print(
            f"Robot speed = {ROBOT_SPEED}%"
        )
        print(
            f"AprilTag = 36h11 / ID {APRILTAG_ID}"
        )
        print()
        print(
            "Robot motion is ENABLED."
        ...
        print(
            "Automatic sequence starts immediately."
        )
        print("========================================")
        ...
        print(
            "[CAMERA] RealSense started."
        )

        set_camera_view_status(
            "CAMERA READY - AUTOMATIC SEQUENCE"
        )
        start_camera_view()
        ...
        print(
            f"[GRIPPER] Prepare/open: "
            f"pos={GRIPPER_PREP_POS}"
        )
        ...
        # ====================================================
        # STEP 1: SAM3 CENTERING
        # ====================================================
        print()
        print("========================================")
        print("STEP 1: SAM3 CRATE CENTERING")
        print("========================================")
        ...
        while True:
            ...
            result, keep_running = (
                acquire_crate_result(
                    predictor,
                    pipeline,
                    align,
                    intrinsics,
                    "SAM3 - searching pink crate",
                )
            )
        ...
        for iteration in range(
            ...
            current_depth_m = (
                float(result["camera_z"])
                / 1000.0
            )

            if (
                current_depth_m < MIN_DEPTH_M
                or current_depth_m > MAX_DEPTH_M
            ):
                raise RuntimeError(
                    "Invalid SAM3 depth."
                )
        ...
        print(
            ...
            f"X={final_error_x:+.1f}, "
            f"Y={final_error_y:+.1f} px"
        )

        if (
            abs(final_error_x)
            > FINAL_CHECK_TOLERANCE_PX
            or
            abs(final_error_y)
            > FINAL_CHECK_TOLERANCE_PX
        ):
            raise RuntimeError(
                "SAM3 final centering check failed."
            )

        print(
            "[FINAL CHECK] SAM3 centering confirmed."
        )

        # ====================================================
        # STEP 2: CURRENT CENTER -> PICKUP
        # ====================================================
        print()
        print("========================================")
        print("STEP 2: MOVE TO PICKUP")
        print("========================================")

        pickup_delta = (
            PICKUP_WOBJ_TARGET
            - CENTERED_WOBJ_REFERENCE
        )

        centered_pose = get_tcp_pose(
            robot
        )
        ...
        print(
            f"[GRIPPER] Close / Pickup: "
            f"pos={GRIPPER_PICK_POS}"
        )
        ...
        move_wobj_target(
            robot,
            lift_target,
            "safe observation height",
        )

        print(
            "[PICKUP] Pickup and lift complete."
        )

        # ====================================================
        # STEP 5: MOVE TO EXPECTED TAG AREA
        # ====================================================
        print()
        print("========================================")
        print("STEP 5: MOVE TO EXPECTED TAG AREA")
        print("========================================")

        print(
            "[MOVE] Applying measured "
            "Pickup -> Tag expected displacement:"
        )
        print(
            f"       X={PICKUP_TO_TAG_EXPECTED_DELTA[0]:+.3f} mm"
        )
        print(
            f"       Y={PICKUP_TO_TAG_EXPECTED_DELTA[1]:+.3f} mm"
        )
        print(
            f"       Z={PICKUP_TO_TAG_EXPECTED_DELTA[2]:+.3f} mm"
        )

        # This is a WObj-frame measured displacement. Since the recorded
        # orientations are effectively identical, apply it as a WObj
        # Cartesian relative target.
        expected_tag_pose = get_tcp_pose(
            robot
        )

        expected_tag_pose[0:3] += (
            PICKUP_TO_TAG_EXPECTED_DELTA
        )
        ...
        # ====================================================
        # STEP 6: RECTANGLE SEARCH
        # ====================================================
        tag = search_apriltag_rectangle(
            robot
        )

        if tag is None:
            raise RuntimeError(
                "AprilTag ID18 was not found."
            )

        # ====================================================
        # STEP 7: CENTER TAG
        # ====================================================
        if not center_apriltag(
            robot
        ):
            raise RuntimeError(
                "AprilTag centering failed."
            )

        # Fresh final Tag check.
        final_tag, keep_running = (
            check_tag_and_display(
        ...
        print(
            ...
            f"({final_tag['center_x']:.0f}, "
            f"{final_tag['center_y']:.0f})"
        )
        print(
            f"[FINAL CHECK] Pixel error = "
            f"X={final_tag_error_x:+.1f}, "
            f"Y={final_tag_error_y:+.1f}"
        )

        if (
            abs(final_tag_error_x)
            > FINAL_CHECK_TOLERANCE_PX
            or
            abs(final_tag_error_y)
            > FINAL_CHECK_TOLERANCE_PX
        ):
            raise RuntimeError(
                "AprilTag final centering check failed."
            )

        # ====================================================
        # STEP 8: TAG CENTER -> BASKET FLOOR
        # ====================================================
        print()
        print("========================================")
        print("STEP 8: MOVE TO BASKET FLOOR")
        print("========================================")

        print(
            "[PLACE] Fixed Tag-center -> basket-floor offset:"
        )
        print(
            f"        X={TAG_TO_BASKET_FLOOR_DELTA[0]:+.3f} mm"
        )
        print(
            f"        Y={TAG_TO_BASKET_FLOOR_DELTA[1]:+.3f} mm"
        )
        print(
        ...
        # IMPORTANT:
        # Do NOT move diagonally from Tag center to the basket floor.
        # First move only in X/Y while keeping the current Z height.
        place_center_pose = get_tcp_pose(robot)

        place_xy_target = place_center_pose.copy()
        place_xy_target[0] += TAG_TO_BASKET_FLOOR_DELTA[0]
        place_xy_target[1] += TAG_TO_BASKET_FLOOR_DELTA[1]
        ...
        # After XY positioning is complete, move only along Z.
        place_z_target = get_tcp_pose(robot)
        place_z_target[2] += TAG_TO_BASKET_FLOOR_DELTA[2]
        ...
        print(
            f"[GRIPPER] Release/open: "
            f"pos={GRIPPER_PREP_POS}"
        )

        # IMPORTANT:
        # Use the same startup/open constant (15), as requested.
        move_gripper(
            robot,
            GRIPPER_PREP_POS,
        )
        ...
        # ====================================================
        # STEP 10: RETURN TO START POSITION
        # ====================================================
        return_to_start_pose(robot)

        print()
        print("========================================")
        print("PICKUP -> TAG -> PLACEMENT -> RETURN COMPLETE")
        print("========================================")
