...
- 이 브랜치의 `fr5-web` 코드는 2026-09-07 16:31 스냅샷입니다. 09-10 S0–S4 실기 결과를 만든 후속 코드는 원본 FR5Web 작업공간에 있으며 아직 이 브랜치로 옮기지 않았습니다.
- 09-11의 S2 자동 재탐색은 접근 거리·요각 후보를 바꾸는 작업 단계입니다. 아직 새 실기 촬영과 배포 확인 전입니다.
- S4의 집기·들기는 한 작업 셀에서 확인했습니다. S5 이후 운반·놓기·반복 사이클은 완료로 표시하지 않습니다.
- 09-07 고스트 정합은 한 자세의 화면 비교입니다. 다양한 자세·조명에서의 반복 오차는 더 측정해야 합니다.
- 2026-09-11 빠른 게이트에서 `grasp-rank`, `sim-batch` 두 검사가 기존 실측·픽스처 차이로 실패했습니다. `agree`는 로봇이 연결되지 않아 판정 재료가 없었습니다. 전체 초록으로 표현하지 않습니다.
