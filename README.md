...
- 이동한 TurtleBot과 거치대를 다시 찾지 못하면 로봇팔 목표가 곧 뒤쳐집니다.
- 숫자만 있는 계획으로는 실제 장비가 어디로 움직일지 사용자가 함께 확인하기 어려웠습니다.

### 목표

1. 작업 셀을 브라우저에서 편집하고 실제 공간에 AR로 겹쳐 봅니다.
2. 글로벌 카메라와 뎁스카메라로 이동한 로봇과 작업물을 다시 관측합니다.
3. 예정 자세를 파란색 고스트와 경로로 먼저 보여주고, 안전 검사를 통과한 동작만 FR5에 전달합니다.
...
화면에서만 보던 작업 셀 맵을 바닥 AR로 불러왔습니다. 작업대 세 개와 안전영역도 같은 좌표 사슬에 넣어, 옮기기 전에 충돌을 확인하는 구조로 확장했습니다.
...
### 3. 배치 안으로 직접 걸어 들어가 확인 — WebXR 답사

바닥에 배치를 띄우는 것만으로는 설비 사이의 통로와 작업 시야를 체감하기 어려웠습니다. 답사 모드는 가상 작업 셀을 실물 1:1 크기로 놓고, 한 번 탭한 지점을 입구로 삼아 실제 걷는 방향으로 펼칩니다. 작업대를 옮기기 전에 컨베이어 주변 통로, 로봇 접근 공간, 시야 가림을 사람의 눈높이에서 확인할 수 있습니다.

<p align="center">
  <img src="assets/media/fr5/xr-workcell-walkthrough.gif" width="360" alt="실제 복도 위에 1대1 크기의 가상 작업 셀을 배치하고 안으로 걸어 들어가는 WebXR 답사 화면">
</p>

### 4. 저장 경로로 주행하고 실제 도착을 확인 — TurtleBot

처음에는 시뮬레이션 고스트만 TurtleBot을 따라갔습니다. 이후 웨이포인트로 저장한 경로를 TurtleBot 브리지에서 실행하고, 위치·속도·주행 상태를 다시 받아 실제 도착을 확인하도록 연결했습니다. 명령을 보냈다는 응답만으로 다음 단계로 넘어가지 않고 `주행 종료 → 속도 0 → 목표 오차 안`을 모두 확인합니다.

| 경로 계획 | 주행 | 안전 감시 | 도착 후 연동 |
|---|---|---|---|
| 정차 후보를 비교하고 저장 경로 선택 | ROS 2 브리지가 웨이포인트 순서대로 이동 | 속도 상한·500ms 워치독·지오펜스·항상 열리는 정지 명령 | 실제 위치를 트윈과 FR5 작업 목표에 다시 반영 |

<table width="100%">
  <tr>
    <td width="50%" align="center"><img src="assets/media/fr5/turtlebot-drive-target.png" width="100%" alt="검은색 현재 TurtleBot과 초록색 이동 목표를 함께 표시한 디지털 트윈 화면"></td>
    <td width="50%" align="center"><img src="assets/media/fr5/2026-09-04-live-follow.gif" width="100%" alt="TurtleBot 이동 뒤 실제 FR5와 디지털 트윈이 새 위치를 추종하는 화면"></td>
  </tr>
  <tr>
    <td align="center"><sub>현재 위치와 정차 목표를 실행 전에 비교</sub></td>
    <td align="center"><sub>이동한 위치를 다시 관측해 FR5 목표 갱신</sub></td>
  </tr>
</table>

### 5. 이동 경로를 실행 전에 보이게 함 — 09-07
...
## 실기 전에 답하는 세 가지 — MuJoCo 시뮬레이션

디지털 트윈은 현재 상태와 예정 동작을 보여주고, MuJoCo는 그 동작을 따라가는 동안 실제 형상이 서로 닿는지 검사합니다. 최종 실행 권한은 시뮬레이션이 아니라 안전 브리지와 현장 작업자에게 남습니다.

| 1. 도달 가능한가 | 2. 허용된 공간인가 | 3. 이동 중 부딪히는가 | 통과 후 |
|---|---|---|---|
| 각 자세의 IK 계산 | 실기와 같은 작업영역 규칙 대조 | 보간한 전체 경로의 로봇·작업대·거치대·총알 접촉 검사 | 고스트로 먼저 재생하고 승인된 동작만 실행 후보로 전달 |

<p align="center">
  <img src="fr5-web/docs/diagrams/sim-data-flow.svg" width="100%" alt="관측값과 실측 설정이 IK, 안전 게이트, MuJoCo 접촉 검사를 거쳐 디지털 트윈 재생으로 이어지는 데이터 흐름">
</p>

파지와 운반의 전체 순서는 [시뮬레이션 사이클 다이어그램](fr5-web/docs/diagrams/sim-cycle.svg)에서 확인할 수 있습니다. 로컬 접촉 검사는 11/11을 통과했습니다. 전체 시뮬 화면 검사는 연속 실행에서 112/114와 114/114로 달라지는 S4 타이밍 민감성이 있고, 현재 배치 일괄검사는 작업영역·충돌 조건에서 0/8이므로 성공률로 표현하지 않습니다.
...
| S1 이동 | 저장 경로, 지오펜스, TurtleBot 위치·속도·주행 상태 | 주행 종료·속도 0·목표 오차 안을 확인한 뒤에만 다음 단계 허용 |
...
- TurtleBot의 최적 경로 생성과 자율주행 알고리즘은 팀원 영역입니다. 이 저장소는 저장 경로 실행, 상태 관제, 안전 정지, FR5 단계 연동을 담당합니다.
